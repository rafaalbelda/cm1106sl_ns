...
async def to_code(config):
    ...
    iaq_num = await sensor.new_sensor(
        {CONF_NAME: f"{config[CONF_NAME]} IAQ (1-5)"}
    )
    iaq_txt = await text_sensor.new_text_sensor(
        {CONF_NAME: f"{config[CONF_NAME]} IAQ Texto"}
    )
